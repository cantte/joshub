...
const CustomerField: FC<Props> = ({ onSelected }) => {
  ...
  const { isFetching, error, data: customer } = useQuery<Customer>(
    ['customer', customerId],
    async () => {
  ...
  return (
    <>
      ...
      {(customer != null) && (
        <div
          className='p-4 mt-3 w-full text-gray-500 bg-white rounded-lg border dark:bg-gray-800 dark:text-gray-400'>
          <div className='flex'>
            <div
              className='inline-flex flex-shrink-0 justify-center items-center w-8 h-8 text-blue-500 bg-blue-100 rounded-lg dark:text-blue-300 dark:bg-blue-900'>
            ...
            <div className='ml-3 text-sm font-normal'>
                    <span
                      className='mb-1 text-sm font-semibold text-gray-900 dark:text-white'>{customer.id}</span>
